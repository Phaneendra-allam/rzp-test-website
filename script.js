...
function toggleCart() { document.getElementById("cartSidebar").classList.toggle("show"); }
...
payButton.addEventListener("click", () => {
  if (!customerNameInput.value || !customerNumberInput.value) { alert("Please fill all details"); return; }
  closeCustomerForm();
  payNow(selectedAmount, selectedProduct, customerNameInput.value, customerNumberInput.value);
});

// ================= PAY FUNCTION =================
function payNow(amount, productName, customerNameVal, customerNumberVal) {
  const options = {
    key: keyId,
    amount: amount * 100,
    currency: "INR",
    ...
    handler: function(response) {
      emailjs.send("service_2l3l97q", "template_zwe1s48", {
        ...
        amount_paid: amount,
        customer_number: customerNumberVal,
